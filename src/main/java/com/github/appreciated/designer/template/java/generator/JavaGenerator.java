...
public class JavaGenerator {
    ...
    public JavaGenerator(DesignCompilerInformation designCompilerInformation) {
        ...
        if (designCompilerInformation.getDesign().getName().endsWith(".java")) {
            ...
            compilationUnit.addOrphanComment(
                    new JavadocComment(
                            " * ! DO NOT EDIT THIS FILE !\n" +
                                    " * <p>\n" +
                                    " * This class was generated by com.github.appreciated.designer.template.java.generator.JavaGenerator.\n" +
                                    " * <p>\n" +
                                    " * Manual changes are not supported.\n" +
                                    " * Generated at " + ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT) + "\n"
                    )
            );
    ...
    private FieldDeclaration addComponent(Component component) {
        ...
        if (designCompilerInformation.hasCompilationMetaInformation(actualComponent) && designCompilerInformation.getCompilationMetaInformation(actualComponent).isProjectComponent()) {
            ...
            field = componentClass.addFieldWithInitializer(
                    ...
                    getVariableNameForProjectComponent(actualComponent), creation
            );
        } else {
            // All other Components
            creation.setType(actualComponent.getClass());
            field = componentClass.addFieldWithInitializer(actualComponent.getClass(), getVariableNameForNoneProjectComponent(actualComponent), creation);
    ...
    private String getVariableNameForProjectComponent(Component actualComponent) {
        if (designCompilerInformation.getCompilationMetaInformation(actualComponent).hasVariableName()) {
            return designCompilerInformation.getCompilationMetaInformation(actualComponent).getVariableName();
        }
        return designCompilerInformation.getCompilationMetaInformation(actualComponent).getClassName() + getCounterForComponent(designCompilerInformation.getCompilationMetaInformation(actualComponent).getClassName());
    }

    private String getVariableNameForNoneProjectComponent(Component actualComponent) {
        if (designCompilerInformation.hasCompilationMetaInformation(actualComponent) && designCompilerInformation.getCompilationMetaInformation(actualComponent).hasVariableName()) {
            return designCompilerInformation.getCompilationMetaInformation(actualComponent).getVariableName();
        }
        return getGeneratedFieldName(actualComponent) + getCounterForComponent(actualComponent);
    }
